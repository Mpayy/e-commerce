...
- [x] **Saga Compensation (Ledger-Based Idempotent Rollback)** — BulkDecreaseStock and BulkRestoreStock both take a generic `idempotency_key` plus the item list directly (not looked up from a ledger) — generalized from an earlier checkout_id-only design so the same RPC serves both checkout compensation (`idempotency_key` = fresh UUID per checkout attempt) and admin order cancellation (`idempotency_key = "cancel-order-{order_id}"`, permanent per order so repeated cancel attempts can't double-restore). Each call writes a `:decrease` or `:restore` ledger entry keyed by the idempotency key.
...
- [x] **Sales Analytics (Admin)** — `GET /admin/analytics/sales:` Daily revenue with a running total via `SUM() OVER (ORDER BY date)` combined with CTEs, and top-selling products via `DENSE_RANK() OVER (...)` — both generated by sqlc; defaults to the last 30 days if `from`/`to` parameters are omitted
...
- [x] **Passthrough Proxy** — Gateway does NOT perform authentication or request transformation; only rate limiting is enforced at this layer. `Authorization` header and all other headers are forwarded verbatim to the target service.
- [x] **Rate Limiting (Token Bucket, per-IP)** — `golang.org/x/time/rate`; keyed by `RemoteAddr` (NOT `X-Forwarded-For`, which is untrusted without a trusted reverse proxy in front of the gateway); stricter limits for `/login` and `/register` (5 req/min — brute-force + bcrypt CPU load) vs general endpoints (60 req/min); `/health` is completely exempt from rate limits; responds with `429` including `X-RateLimit-Limit`/`X-RateLimit-Remaining` headers; inactive client entries are cleaned up by a background goroutine that stops alongside gateway shutdown
...
| **golang.org/x/time/rate** | — | Token bucket rate limiting middleware (api-gateway) |
| **Masterminds/squirrel** | — | Dynamic SQL query builder for admin order filters (order-service) — coexists with sqlc |
...
sequenceDiagram
    ...
    Note over PS,Mongo: MongoDB session.WithTransaction
    loop for each item in REQUEST (bukan hasil lookup ledger)
        PS->>Mongo: UpdateOne products · $inc stock +qty
    Note right of Mongo: MatchedCount==0 → ErrRecordNotFound, transaction rolls back
    end
...
| `GET` | `/api/v1/admin/orders` | List all orders, dynamic filters (Admin) | ✅ Admin |
| `GET` | `/api/v1/admin/orders/:order_id` | Full order detail, any user (Admin) | ✅ Admin |
| `PATCH` | `/api/v1/admin/orders/:order_id/status` | Cancel order + restore stock (Admin) | ✅ Admin |
